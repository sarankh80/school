<template>
    <div class="service-detail service-main mar-bot">
        <div class="service-image">
            <img v-if="serviceData.attchment_extension == true" :src="serviceData.attchments[0]" alt="image"
                class="img-fluid">
            <div v-else class="player-container">
                <vue-core-video-player :src="serviceData.attchments[0]" :cover="baseUrl + '/images/file.png'"
                    :preload="'auto'"></vue-core-video-player>
            </div>
            <span class="service-fav-item badge badge-1 rounded-pill bg-white">{{ __('messages.mark_as_favourite') }}
                <favorite :servicedata="serviceData.id" :favorited="serviceData.is_favourite"
                    @emit-list="serviceDetail" />
            </span>
        </div>
                ...
                <div class="service-heading-box">
                    <div class="service-title">
                        ...
                        <div class="price d-flex align-items-center">
                            <h4 class="primary-color">{{ serviceData.price_format }}</h4> <small
                                v-if="serviceData.type == 'hourly'">/hr</small> <span v-if="serviceData.discount"
                                class="ps-2 ser-discount">{{ serviceData.discount }}% {{ __('messages.off') }}</span>
                        </div>
                    </div>
                    <div class="service-meta">
                        <ul class="list-inline d-flex flex-wrap align-items-center mb-0">
                            <li class="d-flex align-items-center">
                                <span
                                    class="list-label heading-color heading-weight">{{ __('messages.category') }}:</span><span
                                    class="list-value">
                                    <div class="d-flex align-items-center text-primary px-2">
                                        {{ serviceData.category_name }} <span v-if="serviceData.subcategory_name"><span
                                                class="subcat_spacing"> >
                                            </span><span>{{ serviceData.subcategory_name }}</span></span>
                                    </div>
                                </span>
                            </li>
                            <li class="d-flex align-items-center">
                                <span
                                    class="list-label heading-color heading-weight">{{ __('messages.rating') }}:</span><span
                                    class="list-value">
                                    <div class="d-flex align-items-center text-primary px-2">
                                        <rating :readonly=true :showrating="false"
                                            :ratingvalue="serviceData.total_rating" />
                                    </div>
                                </span>
                            </li>
                        </ul>
                        <div class="book-now-btn">
                            <button class="btn btn-primary">
                                <router-link class="text-white"
                                    :to="{ name: 'book-service', params: { service_id: serviceData.id } }">
                                    {{ __('messages.book_now') }}</router-link>
                            </button>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="service-detail-info">
                            ...
                            <div class="service-gallery service-top-space">
                                <div class="header-title-inner-page d-flex align-items-center justify-content-between">
                                    ...
                                    <router-link
                                        :to="{ name: 'gallery', params: { galleryimg: serviceData.attchments, service_name: serviceData.name } }"
                                        class="link-btn-box text-primary"><span>{{ __('messages.view_all') }}</span>
                                    </router-link>
                                </div>
                                <div class="gallery-box">
                                    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3">
                                        <div v-for="(serviceimg, index) in serviceData.attchments" :key="index"
                                            class="col">
                                    ...
                                    <ul class="list-inline">
                                        <li v-for="(rating, index) in ratingData" :key="index" class="review-card">
                                            <div class="author-wrapper card-details">
                                                <div class="autho-img">
                                                    <img :src="rating.profile_image" alt="image"
                            ...
                            <div v-if="relatedService.length > 0" class="service-related-product service-top-space">
                                ...
                                <div class="relateservice our-service">
                                    <div class="row">
                                        <div class="col-lg-4 col-md-6 col-xs-12" v-for="(data, index) in relatedService"
                                            :key="index">
                                            <service-list :serviceId="data.id"
                                                :imageUrl="data.attchments[0] ? data.attchments[0] : baseUrl + '/images/default.png'"
                                                :is_favourite="data.is_favourite" :servicePrice="data.price_format"
                                                :serviceName="data.name" :serviceRating="data.total_rating"
                                                :serviceDescription="data.description"
                                                :serviceProviderImg="data.provider_image"
                                                :serviceProvider="data.provider_name" :serviceType="data.type"
                            ...
                            <div class="card-body">
                                ...
                                <div class="service-tag">
                                    <ul class="list-inline mb">
                                        <li v-for="(address, index) in serviceData.service_address_mapping"
                                            :key="index">
                                            <a class="active" href="#" @click="getBookingAddress(address.address_id)">{{
                                                    address.provider_address_mapping.address
                                            }}</a>
                                        </li>
                                    </ul>
                                </div>
                                ...
                                <div class="provider-info">
                                    <div class="d-sm-flex d-flex">
                                        <div class="provider-user me-3">
                                            <img :src="provider.profile_image" alt="image"
                                                class="img-fluid avatar avatar-70 rounded-circle">
                                        </div>
                                        <div class="provider-content mt-3 mt-sm-0 w-100">
                                            <div
                                                class="provider-title d-flex align-items-center justify-content-between">
                                            ...
                                            <div class="ratting-value">
                                                <div class="d-flex align-items-center">
                                                    <rating :readonly=true :showrating="false"
                                                        :ratingvalue="provider.providers_service_rating" />
                                                </div>
                                            </div>

                                            <a v-bind:href="'mailto:' + provider.email"
                                                class="btn btn-primary mt-4">{{ __('messages.contact') }}</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card" v-if="serviceData.duration">
                            <div class="card-body">
                                ...
                                <div class="duration-details">
                                    <p>{{ __('messages.hour_service_desc') }}<strong class="text-primary">
                                            {{ serviceData.duration }} {{ __('messages.hour') }} </strong></p>
                                </div>
                            </div>
                        </div>
        ...
        <div class="service-book-now">
            <router-link :to="{ name: 'book-service' }" class="btn btn-primary rounded"> {{ __('messages.book_now') }}
            </router-link>
        </div>
        <b-modal id="reviewedit" modal-class="modal fade" title="Review"
            header-close-content=" <button type='button' class='btn-close btn-sm' data-bs-dismiss='modal' aria-label='Close'></button>">
            <form class="review-form">
                ...
                <button type="submit" class="btn btn-primary mt-3"
                    data-bs-dismiss='modal'>{{ __('messages.update') }}</button>
            </form>
        </b-modal>
    </div>
</template>
