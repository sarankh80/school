...
class Cart 
{
     ...
     * Returns cart.
     *
     * @return App\Contracts\Cart|null
     */
     ...
     */
    public function getCart($cartId = NULL): ? CartModel
    {
        if ($this->cart) {
            return $this->cart;
        }
        ...
        if(isset($cartId))
        {
            ...
            if (auth()->guard()->check()) {
                $this->cart = $this->cartRepository->findLastWhere([
            ...
            if(!empty($this->cart) && $this->cart->booking()->exists())
            {
                session()->forget('cart');
                $this->cart = null;
            }
     ...
     * @return App\Contracts\CartItem|void
     */
     ...
     * @param  int  $productId
     * @param  array  $data
     * @return App\Contracts\Cart|string|array
     * @throws Exception
     */
    public function addProduct($productId, $data, $cartId)
    {
        Event::dispatch('checkout.cart.add.before', $productId);

        $cart = $this->getCart(cartId: $cartId);
        ...
        $product = $this->serviceRepository->skipPresenter()->find($productId);
    ...
    public function addProductItem($productId, $data, $cartId)
    {
        Event::dispatch('checkout.cart.add.before', $productId);
        $cart = null;
        if (! $cart) {
     ...
     */
    public function updateItems($data, $cartId = NULL)
    {
     ...
     * @return boolean
     */
    public function removeItem($itemId, $cartId = NULL)
    {
        Event::dispatch('checkout.cart.delete.before', $itemId);

        if (! $cart = $this->getCart($cartId)) {
            return false;
        }
     ...
     * @return void
     */
    public function collectTotals($cartId = NULL): void
    {
        if (! $this->validateItems($cartId)) {
            return;
        }
        ...
        // Event::dispatch('checkout.cart.collect.totals.before', $cart); // uncoment when using discount from cart rule and catalog rule
        ...
        $cart->grand_total = $cart->sub_total + $cart->tax_total;
     ...
     * @return void
     */
    public function calculateItemsTax($cartId = NULL): void
    {
        if (! $cart = $this->getCart($cartId)) {
            return;
        }
     ...
     * @return bool
     */
    public function validateItems($cartId = NULL): bool
    {
        if (! $cart = $this->getCart($cartId)) {
            return false;
        }
        ...
        foreach ($cartItems as $item) {
            ...
            if ($validationResult->isItemInactive()) {
                ...
                $base_price = ! is_null($item->custom_price) ? $item->custom_price : $item->base_price;
                $price = ! is_null($item->custom_price) ? $item->custom_price : $item->price;
                
                $this->cartItemRepository->update([
                    'price'      => core()->convertPrice($price),
                    'base_price' => $base_price,
                    'total'      => core()->convertPrice($price * $item->quantity),
                    'base_total' => $base_price * $item->quantity,
                ], $item->id);
            }
     ...
     * @return array
     */
    public function prepareDataForBooking($cartId = NULL): array
    {
        $data = $this->toArray($cartId);

        $finalData = [
            'cart_id'               => $this->getCart($cartId)->id,
        ...
        foreach ($data['items'] as $item) {
            $finalData['items'][] = $this->prepareDataForBookingItem($item);
        }
     ...
     * @return array
     */
    public function prepareDataForBookingItem($data): array
    {
        $finalData = [
            'service_id'           => $data['product_id'],
            'sku'                  => $data['sku'],
            'type'                 => $data['type'],
            'name'                 => $data['name'],
    ...
    public function prepareDataForOrderItem($data): array
    {
        ...
        $finalData = [
            'product'              => $this->serviceRepository->find($data['product_id']),
            'sku'                  => $data['sku'],
            'type'                 => $data['type'],
            'name'                 => $data['name'],
            'weight'               => $data['weight'],
     ...
     * @return array
     */
    public function toArray($cartId = NULL)
    {
     ...
     * @return bool
     */
    private function isCartItemInactive(\App\Contracts\CartItem $item): bool
    {
