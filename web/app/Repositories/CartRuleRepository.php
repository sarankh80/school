...
use App\Repositories\CartRuleCouponRepository;
...
class CartRuleRepository extends Repository
{
     ...
     * @param  \App\CartRuleCouponRepository  $cartRuleCouponRepository
    ...
    public function getConditionAttributes()
    {
        $attributes = [
            [
                'key'      => 'cart',
                'label'    => trans('messages.promotions.cart-rules.cart-attribute'),
                'children' => [
                    [
                        'key'   => 'cart|base_sub_total',
                        'type'  => 'price',
                        'label' => trans('messages.promotions.cart-rules.subtotal'),
                    ], [
                        'key'   => 'cart|items_qty',
                        'type'  => 'integer',
                        'label' => trans('messages.promotions.cart-rules.total-items-qty'),
                    ], [
                        'key'     => 'cart|payment_method',
                        'type'    => 'select',
                        'options' => $this->getPaymentMethods(),
                        'label'   => trans('messages.promotions.cart-rules.payment-method'),
                    ], [
                        'key'     => 'cart|shipping_method',
                        'type'    => 'select',
                        'options' => $this->getShippingMethods(),
                        'label'   => trans('messages.promotions.cart-rules.shipping-method'),
                    ], [
                        'key'   => 'cart|postcode',
                        'type'  => 'text',
                        'label' => trans('messages.promotions.cart-rules.shipping-postcode'),
                    ], [
                        'key'     => 'cart|state',
                        'type'    => 'select',
                        'options' => $this->groupedStatesByCountries(),
                        'label'   => trans('messages.promotions.cart-rules.shipping-state'),
                    ], [
                        'key'     => 'cart|country',
                        'type'    => 'select',
                        'options' => $this->getCountries(),
                        'label'   => trans('messages.promotions.cart-rules.shipping-country'),
                    ],
                ],
            ], [
                'key'      => 'cart_item',
                'label'    => trans('messages.promotions.cart-rules.cart-item-attribute'),
                'children' => [
                    [
                        'key'   => 'cart_item|base_price',
                        'type'  => 'price',
                        'label' => trans('messages.promotions.cart-rules.price-in-cart'),
                    ], [
                        'key'   => 'cart_item|quantity',
                        'type'  => 'integer',
                        'label' => trans('messages.promotions.cart-rules.qty-in-cart'),
                    ], [
                        'key'   => 'cart_item|base_total_weight',
                        'type'  => 'decimal',
                        'label' => trans('messages.promotions.cart-rules.total-weight'),
                    ], [
                        ...
                        'type'  => 'text',
                        'label' => trans('messages.promotions.cart-rules.additional'),
                    ],
                ],
            ], [
                'key'      => 'product',
                'label'    => trans('messages.promotions.cart-rules.product-attribute'),
                'children' => [
                    [
                        'key'     => 'product|category_ids',
                        'type'    => 'multiselect',
                        'label'   => trans('messages.promotions.cart-rules.categories'),
                        'options' => $categories = $this->categoryRepository->getCategoryTree(),
                    ], [
                        'key'     => 'product|children::category_ids',
                        'type'    => 'multiselect',
                        'label'   => trans('messages.promotions.cart-rules.children-categories'),
                        'options' => $categories,
                    ], [
                        'key'     => 'product|parent::category_ids',
                        'type'    => 'multiselect',
                        'label'   => trans('messages.promotions.cart-rules.parent-categories'),
                        'options' => $categories,
                    ], [
                        'key'     => 'product|attribute_family_id',
                        'type'    => 'select',
                        'label'   => trans('messages.promotions.cart-rules.attribute_family'),
        ...
        foreach ($this->attributeRepository->findWhereNotIn('type', ['textarea', 'image', 'file']) as $attribute) {
            ...
            $attributes[2]['children'][] = [
                ...
                'type'    => $attribute->type,
                'label'   => trans('messages.promotions.cart-rules.attribute-name-children-only', ['attribute_name' => $attribute->name]),
                'options' => $options,
            ];
            ...
            $attributes[2]['children'][] = [
                ...
                'type'    => $attribute->type,
                'label'   => trans('messages.promotions.cart-rules.attribute-name-parent-only', ['attribute_name' => $attribute->name]),
                'options' => $options,
            ];
        }
