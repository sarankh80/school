{{ Form::open(['route' => ['categories.destroy', $category->id], 'method' => 'delete', 'data--submit' => 'category' . $category->id]) }}
<div class="d-flex justify-content-end align-items-center">
    @if (!$category->trashed())
        @can('category edit')
            <a class="mr-2" href="{{ route('categories.edit', $category->id) }}"
                title="{{ __('messages.update_form_title', ['form' => __('messages.category')]) }}"><i
                    class="fas fa-pen text-secondary"></i></a>
        @endcan

        @can('category delete')
            <a class="mr-2" href="javascript:void(0)" data--submit="category{{ $category->id }}" data--confirmation='true'
    ...
    @if (auth()->user()->hasAnyRole(['admin']) && $category->trashed())
        <a href="{{ route('categories.action', ['id' => $category->id, 'type' => 'restore']) }}"
        ...
        <a href="{{ route('categories.action', ['id' => $category->id, 'type' => 'forcedelete']) }}"
