<x-master-layout>
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                {{ Breadcrumbs::render('edit_provider', $provider->id) }}
            </div>
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        {{ Form::model($provider, ['method' => 'PUT', 'route' => ['providers.update', $provider->id], 'enctype' => 'multipart/form-data', 'data-toggle' => 'validator', 'id' => 'provider']) }}
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                {{ Form::select(
                                    'providertype_id',
                                    [optional($provider->providertype)->id => optional($provider->providertype)->name],
                                    optional($provider->providertype)->id,
                                    [
                            ...
                            <div class="form-group col-md-4">
                                ...
                                {{ Form::select(
                                    'country_id',
                                    [optional($provider->country)->id => optional($provider->country)->name],
                                    optional($provider->country)->id,
                                    [
                            ...
                            @if (getMediaFileExit($provider, 'profile_image'))
                                <div class="col-md-2 mb-2">
                                    <img id="profile_image_preview"
                                        src="{{ getSingleMedia($provider, 'profile_image') }}" alt="#"
                                        class="attachment-image mt-1">
                                    <a class="text-danger remove-file"
                                        href="{{ route('remove.file', ['id' => $provider->id, 'type' => 'profile_image']) }}"
                        ...
                        <div class="row">
                            <div class="form-group col-md-6">
                                <div class="custom-control custom-checkbox custom-control-inline">
                                    {{ Form::checkbox('is_featured', $provider->is_featured, null, ['class' => 'custom-control-input', 'id' => 'is_featured']) }}
    ...
    @php
        $data = $provider->taxes->pluck('id')->implode(',');
    @endphp
        ...
        <script type="text/javascript">
            (function($) {
                ...
                $(document).ready(function() {
                    var country_id = "{{ isset($provider->country_id) ? $provider->country_id : 0 }}";
                    var state_id = "{{ isset($provider->state_id) ? $provider->state_id : 0 }}";
                    var city_id = "{{ isset($provider->city_id) ? $provider->city_id : 0 }}";

                    var provider_id = "{{ isset($provider->id) ? $provider->id : '' }}";
