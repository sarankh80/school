...
function tranSlateLaguage($id,$request,$type)
{
    $tran = TranslateLanguage::where('type',$type)->where('re_id',$id)->first() ? TranslateLanguage::where('type',$type)->where('re_id',$id)->first() : new TranslateLanguage();
    $tran->re_id = $id; 
    $tran->type = $type;
    ...
    if(!empty($request->description)){
        $tran->setTranslation('des', 'en', $request->description);
        $tran->setTranslation('des', 'km', $request->description_km);
        $tran->setTranslation('des', 'zh', $request->description_zh);
    }
    if(!empty($request->included)){
        $tran->setTranslation('included', 'en', $request->included);
        $tran->setTranslation('included', 'km', $request->included_km);
        $tran->setTranslation('included', 'zh', $request->included_zh);
    }
    if(!empty($request->excluded)){
        $tran->setTranslation('excluded', 'en', $request->excluded);
        $tran->setTranslation('excluded', 'km', $request->excluded_km);
        $tran->setTranslation('excluded', 'zh', $request->excluded_zh);
    }  
    $tran->status = 1;
    $tran->save();
}
...
function saveBookingActivity($data) 
{
    ...
    switch ($data['activity_type'])
    {
        case "add_booking":
                ...
                $address = \App\Models\Address::find($data['booking']->booking_address_id);
                $sendTo = ['admin' , 'provider'];
                $group_id = "@gogospidernewbooking";
                $token_id = "6947126886:AAE9mMKQp2BlAj-H1pKKAO6a2RqYQQf8LL4";
                $body = $data['activity_type']." ID: ". $data['booking_id']." ". $data['activity_message']." Customer Name : ".
                $data['booking']->customer->display_name." Phone Number : ".
                $data['booking']->customer->phone_number." Maps  : https://www.google.com/maps/place/$address->latitude,$address->longitude".
                " Link Item  : https://gogospider.com/history-booking/".$data['booking']->id; 
                sendTelegram($group_id,$body,$token_id);
            break;
        case "accept_booking":
            ...
            $data['activity_type'] = __('messages.book_id')." ".$data['booking_id'].' '.__('messages.accept');
            // $data['activity_message'] = __('messages.booking_status_update',[ 'from' => $old_status , 'to' => $status ]);
            $data['activity_message'] =  __('messages.booking_id')." ".$data['booking_id'].' ('.__('messages.accept').')';
            $activity_data = [
        ...
        case "update_booking_status":
                ...
                $old_status = \App\Models\BookingStatus::bookingStatus($data['booking']->old_status);
                // $data['activity_type'] = __('messages.update_booking_status');
                $data['activity_type'] = __('messages.book_id')." ".$data['booking_id'].' '.$status;
                // $data['activity_message'] = __('messages.booking_status_update',[ 'from' => $old_status , 'to' => $status ]);
                $data['activity_message'] = __('messages.booking_id')." ".$data['booking_id'].' ('.$status.')';
                $activity_data = [
                    'service_id' => $data['booking']->service_id,
                    'service_name' => isset($data['booking']->service) ? $data['booking']->service->name : '',
                    'reason' => $data['booking']->reason,
        ...
        case "payment_message_status" :
            ...
            $data['activity_type'] = __('messages.book_id')." ".$data['booking_id'].' '.__('messages.paid');
            $data['activity_message'] = __('messages.booking_id')." ".$data['booking_id'].' ('.__('messages.paid').')';
    ...
    foreach($sendTo as $to){
        switch ($to)
        {
            case 'admin':
                ...
                break;
            case 'provider':
                $playerIds = !empty($data['playerIds']) ? $data['playerIds'] : [];
        ...
        // $group_id = "407851810";
        // $group_id = "-4037139364";
        // $group_id = "-1002114601664";
              
        if($to != 'handyman' && (!empty($user->player_id)) || !empty($playerIds)) {
            sendNotification($to, $user, $notification_data, $playerIds);               
        }
    }

}
function sendTelegram($Notifyid,$body,$token){        
    $token = $token;
    $to = "";
    $Notifyid = $Notifyid;
    $data = [
...
function adminEarning(){
    ...
    foreach ($revenuedata as $key => $value) {
       ...
       if(!empty($booking)){
        ...
        $provider_commission = optional(optional($provider)->providertype)->commission;
        $provider_type = optional(optional($provider)->providertype)->type;
...
function sendNotification($type,$user,$data, $playerIds=[]){
    $app_id = config('onesignal.onesignal_api_key');
    $rest_api_key = config('onesignal.onesignal_rest_api_key');
    if($type === 'user'){
        $app_id = config('onesignal.onesignal_api_key');
        $rest_api_key = config('onesignal.onesignal_rest_api_key');
        }
    if($type === 'provider'){
        $app_id = config('onesignal.onesignal_api_key_provider');
        $rest_api_key = config('onesignal.onesignal_rest_api_key_provider');
    }
    $heading      = array(
    ...
    $fields = array(
        'app_id' => $app_id,
        'include_player_ids' => !empty($playerIds) ? $playerIds : array($user->player_id),
        'data' =>  array(
            'type' => $data['type'],
            'id' => $data['id']
        ),
    ...
    if(!empty($user))
    {
        ...
        if(!empty($playerIds))
        {
            $booking = \App\Models\Booking::find($data['id']);
            $requestProviders = $booking->requestProvider;
