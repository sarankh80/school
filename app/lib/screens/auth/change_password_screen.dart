  ...
  Future<void> changePassword() async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      await changeUserPassword(request).then((res) async {
        print("res change : $res");
        await authService.changePassword(newPasswordCont.text).then((value) {
          userService
              .getUserEmail(email: getStringAsync(USER_EMAIL))
              .then((valuer) async {
            authService.updateUserDataPss(
                valuer, valuer.id, newPasswordCont.text);
          });
        }).catchError((e) {
          log(e.toString());
        });
