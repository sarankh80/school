...
class CartRuleCouponController extends Controller
{
    ...
    public function store($id)
    {
        ...
        if (! request('id')) {
            return response()->json(['message' => trans('messages.promotions.cart-rules.cart-rule-not-defind-error')], 400);
        }

        $this->cartRuleCouponRepository->generateCoupons(request()->all(), request('id'));

        return response()->json(['message' => trans('messages.response.create-success', ['name' => 'Cart rule coupons'])]);
    }
