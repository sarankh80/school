                ...
                <form method="POST" action="{{ route('admin.reset_password.store') }}" @submit.prevent="onSubmit">
                    ...
                    <div class="control-group" :class="[errors.has('email') ? 'has-error' : '']">
                        ...
                        <input type="text" v-validate="'required|email'" class="control" id="email" name="email"
                            data-vv-as="&quot;{{ __('admin::app.users.reset-password.email') }}&quot;"
                            value="{{ old('email') }}" />
                    ...
                    <div class="control-group" :class="[errors.has('password') ? 'has-error' : '']">
                        ...
                        <input type="password" v-validate="'required|min:6'" class="control" id="password" name="password"
                            ref="password" data-vv-as="&quot;{{ __('admin::app.users.reset-password.password') }}&quot;" />
                        <span class="control-error" v-if="errors.has('password')">@{{ errors.first('password') }}</span>
                    </div>

                    <div class="control-group" :class="[errors.has('password_confirmation') ? 'has-error' : '']">
                        <label
                            for="password_confirmation">{{ __('admin::app.users.reset-password.confirm-password') }}</label>
                        <input type="password" v-validate="'required|min:6|confirmed:password'" class="control"
                            id="password_confirmation" name="password_confirmation"
                            data-vv-as="&quot;{{ __('admin::app.users.reset-password.confirm-password') }}&quot;"
                            data-vv-as="password" />
                    ...
                    <div class="button-group">
                        <button type="submit"
                            class="btn btn-xl btn-primary">{{ __('admin::app.users.reset-password.submit-btn-title') }}</button>
                    </div>
