...
abstract class _AppStore with Store {
  ...
  @observable
  bool isAlertChooseLocation = false;

  @observable
  bool isHomeCategory = true;
  ...
  @action
  Future<void> setCustomeLocation(bool val,
      {bool isInitializing = false}) async {
    isCustomeLocation = val;
    if (!isInitializing) await setValue(IS_CUSTOME_LOCATION, val);
  }

  @action
  Future<void> setSavedLocation(bool val, {bool isInitializing = false}) async {
    isSavedLocation = val;
    if (!isInitializing) await setValue(IS_SAVED_LOCATION, val);
  }

  @action
  Future<void> setSavedAddress(List<SavedAddress> val) async {
    await setValue(SAVED_ADDRESS, val);
  }

  @action
  Future<void> setBookingInfo(BookingInfo val) async {
    await setValue(BOOKING_INFO, val);
  }

  @action
  Future<void> setAlertChooseLocation(bool val,
      {bool isInitializing = false}) async {
    isAlertChooseLocation = val;
    if (!isInitializing) await setValue(ALERT_CHOOSE_LOCATION, val);
  }

  @action
  Future<void> setHomeCategory(bool val, {bool isInitializing = false}) async {
    isHomeCategory = val;
    if (!isInitializing) await setValue(IS_HOME_CATEGORY, val);
  }
  ...
  @action
  void setNotificationCount(int val) async {
