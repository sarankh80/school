<script>
    (function($) {
        ...
        $(document).ready(function() {
            ...
            function confirmation(form, title = "{{ __('messages.confirmation') }}", message =
                "{{ __('messages.delete_msg') }}", ajaxtype = false, _this) {
                $.confirm({
                    content: message,
                    type: '',
                    title: title,
                            ...
                            action: function() {
                                ...
                                if (ajaxtype == 'true') {
                                    ...
                                    $.post(url, data).then(response => {
                                        if (response.status) {
                                            ...
                                            if (response.image != null) {
                                                $(_this).remove();
                                                $('#' + response.preview).attr('src',
                                                    response.image)
                                                if (jQuery.inArray(response.preview, [
                                                        "service_attachment_preview"
                                                    ]) !== -1) {
                                                    $('#' + response.preview + "_" +
                                                        response.id).remove()
                                                    let total_file = $('.remove-file')
                                                        .length;
                                                    if (total_file == 0) {
                                                        $('.service_attachment_div')
                                                            .remove();
                                                    }
                                                }
                                                if (response.preview ==
                                                    'site_logo_preview') {
                                                    $('.' + response.preview).attr(
                                                        'src', response.image);
                                                }
                                                if (response.preview ==
                                    ...
                                    if (form !== undefined && form) {
                                        $(document).find('[data--submit="' + form + '"]')
                                            .submit();
                                    } else {
                                        return true;
                                    }
