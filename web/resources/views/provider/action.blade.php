...
?>
{{ Form::open(['route' => ['providers.destroy', $provider->id], 'method' => 'delete', 'data--submit' => 'provider' . $provider->id]) }}
<div class="d-flex justify-content-end align-items-center">
    @if (!$provider->trashed())
        @if ($auth_user->can('provider edit'))
            <a class="mr-2" href="{{ route('providers.edit', $provider->id) }}"
                title="{{ __('messages.update_form_title', ['form' => __('messages.provider')]) }}"><i
                    class="fas fa-pen text-secondary"></i></a>
        @endif
        <a class="mr-2" href="{{ route('providers.show', $provider->id) }}"><i class="far fa-eye text-secondary"></i></a>
    ...
    @if (auth()->user()->hasAnyRole(['admin']) && $provider->trashed())
        <a href="{{ route('providers.action', ['id' => $provider->id, 'type' => 'restore']) }}"
        ...
        <a href="{{ route('providers.action', ['id' => $provider->id, 'type' => 'forcedelete']) }}"
