...
class UserController extends Controller
{
    ...
    public function register(UserRequest $request)
    {
        ...
        $input['customer_group_id'] = $this->customerGroupRepository->findOneWhere(['name' => 'general'])->id;
    ...
    public function handymanList(Request $request)
    {
        $user_type = isset($request['user_type']) ? $request['user_type'] : 'handyman';

        $handyman_list = Provider::where('user_type', $user_type)->where('status', 1)->withTrashed();
        if ($request->has('provider_id')) {
            $handyman_list = $handyman_list->where('provider_id', $request->provider_id);
        }
        if ($request->has('city_id') && !empty($request->city_id)) {
