...
abstract class AbstractType
{
    ...
    public function __construct(
        ...
        protected ServiceRepository $productRepository,
    ...
    {
        if (! $this->canBeCopied()) {
            throw new \Exception(trans('messages.response.product-can-not-be-copied', ['type' => $this->product->type]));
        }
    ...
    public function getPriceIndex()
    {
        ...
        $indices[$this->product->id] = $this->product
            ->price_indices
            ->where('customer_group_id', optional($customerGroup)->id)
            ->first();
    ...
    public function prepareForCart($data)
    {
        ...
        $discount = number_format(($this->product->discount * $price) / 100, 2, '.', '');
        ...
        $products = [  
            [
                ...
                'discount_percent'  => !empty($this->product->discount) ? $this->product->discount : 0,
                'discount_amount'   => $discount,
                'price'             => $convertedPrice = core()->convertPrice($price_after_discount),
                'base_price'        => $price,
     ...
     * @return bool
     */
    public function isCartItemInactive(\App\Contracts\CartItem $item): bool
    {
