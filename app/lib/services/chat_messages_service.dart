...
class ChatMessageService extends BaseService {
  ...
  Future<void> addMessageToDb(
    ...
    if (image != null) {
      ...
      Reference storageRef =
          _storage.ref().child("Chat Images").child(fileName);
  ...
  Query fetchChatListQuery({String? userId}) {
    ...
    return userRef!
        ...
        .collection(CONTACT_COLLECTION)
        .orderBy("uid")
        .where("uid", whereNotIn: ["V4oEijjyDTbRqc3yCRpHLNnzVIJ2"]);
  }

  Query fetchChatListSupportQuery({String? userId}) {
    // log("Group User ${userId}");
    return userRef!
        .doc(userId)
        .collection(CONTACT_COLLECTION)
        .where("uid", isEqualTo: "V4oEijjyDTbRqc3yCRpHLNnzVIJ2");
  }

  int getAllUnReadCount({String? senderId, String? documentId}) {
    int count = 0;
    final q = userRef!
        .doc(senderId)
        .collection(CONTACT_COLLECTION)
    ...
    q.forEach((element) {
      element.forEach(
        (value) {
          // Stream<int> b =
          //     getUnReadCount(senderId: senderId, receiverId: value.id);
          ref!
              .doc(senderId.toString())
              .collection(value.id)
              .where('isMessageRead', isEqualTo: false)
