...
<div class="d-flex justify-content-end align-items-center">
    @if(!$booking->trashed())
        @if ($auth_user->can('booking edit'))
            <?php 
                $address = $booking->customerAddress()->find($booking->booking_address_id); 
                $latitude= $address->latitude??"";
                $longitude= $address->longitude??"";
            ?>
            <a class="mr-2" href="https://www.google.com/maps/place/{{$latitude}},{{$longitude}}" target="_blank"
                title="{{ __('messages.view_map', ['form' => __('messages.service')]) }}">
