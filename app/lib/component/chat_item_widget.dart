...
class _ChatItemWidgetState extends State<ChatItemWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (date.day == DateTime.now().day) {
      time = formatDate(widget.chatItemData!.createdAt.validate().toString(),
          format: HOUR_12_FORMAT, isFromMicrosecondsSinceEpoch: true);
    } else {
      time = formatDate(widget.chatItemData!.createdAt.validate().toString(),
    ...
    Widget chatItem(String? messageTypes) {
      switch (messageTypes) {
        ...
        case IMAGE:
          if (widget.chatItemData!.photoUrl.validate().isNotEmpty ||
              widget.chatItemData!.photoUrl != null) {
            return Stack(
              children: [
                CachedImageWidget(
